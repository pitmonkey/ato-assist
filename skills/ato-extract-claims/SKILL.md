...
- **A claim about two table cells cites both.** Where a status in one column and a narrative in another only mean something together, that is two `source:` entries, one per cell — not one quote concatenating them. A concatenated quote is a sentence the document does not contain, and a reader opening the chunk will not find it. This is what multiple source entries are for.
